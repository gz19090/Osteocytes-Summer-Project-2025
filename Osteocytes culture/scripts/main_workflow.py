...
def main(max_frames: int = None, min_area: int = 10, use_percentile: bool = False, percentile: float = 87.0, crop: tuple = None, num_wildtype: int = None, num_mutant: int = None):
    ...
    Args:
        ...
        num_wildtype (int, optional): Number of wildtype videos to process (default: None, all).
        num_mutant (int, optional): Number of mutant videos to process (default: None, all).
    """
        ...
        for video_path in video_paths:
            ...
            for frame_idx, frame in enumerate(frames_to_process):
                ...
                try:
                    ...
                    frame_dir = f'frame_{frame_idx:04d}'
                    frame_output_dir = video_output_dir / frame_dir
                    frame_results_dir = video_results_dir / frame_dir
                    ...
                    save_image(filtered, str(frame_output_dir / f'{frame_prefix}_filtered.tif'))
                    save_image(combined, str(frame_output_dir / f'{frame_prefix}_combined.tif'))
                    save_image(labeled, str(frame_output_dir / f'{frame_prefix}_labeled.tif'))
                    ...
                    plot_histograms(filtered, cell_metrics['area'].tolist(), cell_metrics['dendritic_length'].tolist(), cell_metrics['eccentricity'].tolist(), cell_metrics['solidity'].tolist(), str(frame_results_dir / f'{frame_prefix}_histograms.png'))
                    
                    if cropped is not None:
                        save_image(cropped, str(frame_output_dir / f'{frame_prefix}_cropped.tif'))
                        save_image(combined_cropped, str(frame_output_dir / f'{frame_prefix}_combined_cropped.tif'))
                        save_image(labeled_cropped, str(frame_output_dir / f'{frame_prefix}_labeled_cropped.tif'))
                        plot_edge_filters(cropped, str(frame_results_dir / f'{frame_prefix}_edge_filters_cropped.png'))
                        plot_combined_image(cropped, combined_cropped, weights_cropped, str(frame_results_dir / f'{frame_prefix}_combined_cropped.png'))
                        plot_contours(combined_cropped, contours_cropped, str(frame_results_dir / f'{frame_prefix}_contours_cropped.png'))
                        plot_segmentation(cropped, combined_cropped, labeled_cropped, str(frame_results_dir / f'{frame_prefix}_segmentation_cropped.png'))
                        plot_histograms(cropped, cell_metrics_cropped['area'].tolist(), cell_metrics_cropped['dendritic_length'].tolist(), cell_metrics_cropped['eccentricity'].tolist(), cell_metrics_cropped['solidity'].tolist(), str(frame_results_dir / f'{frame_prefix}_histograms_cropped.png'))
...
if __name__ == '__main__':
    ...
    # Add arguments for limiting number of videos
    ...
    main(max_frames=args.max_frames, min_area=args.min_area, use_percentile=args.use_percentile, percentile=args.percentile, crop=args.crop, num_wildtype=args.num_wildtype, num_mutant=args.num_mutant)
